...
def validate_model_on_test_set_eign(model, dataset, loss_func, device, use_signed=False):
    model.eval()
    total_loss = 0.0
    all_preds = []
    all_targets = []
    ...
    with torch.inference_mode():
        if isinstance(dataset, list):
            for data in dataset:
                ...
                eign_output = model(
                    x_unsigned=(
                        data.x.to(device) if hasattr(data, "x") and data.x is not None else None
                    ),
                    x_signed=(
                        data.x_signed.to(device)
                        if hasattr(data, "x_signed") and data.x_signed is not None
                        else None
                    ),
                    edge_index=data.edge_index.to(device),
                    is_directed=data.edge_is_directed.to(device),
                )
                ...
                targets_node_predictions_signed = data.y_signed.to(device)
                targets_node_predictions_unsigned = data.y.to(device)

                loss = loss_func(
                        predicted_signed,
                        targets_node_predictions_signed
                    ).item() if use_signed else loss_func(
                        predicted_unsigned,
                ...
                all_preds.append(predicted_signed if use_signed else predicted_unsigned)
                all_targets.append(targets_node_predictions_signed if use_signed else targets_node_predictions_unsigned)
        else:
            input_node_features, targets = dataset.x.to(device), dataset.y.to(device)
            eign_output = model(
                x_unsigned=(
                    dataset.x.to(device) if hasattr(dataset, "x") and dataset.x is not None else None
                ),
                x_signed=(
                    dataset.x_signed.to(device)
                    if hasattr(dataset, "x_signed") and dataset.x_signed is not None
                    else None
                ),
                edge_index=dataset.edge_index.to(device),
                is_directed=dataset.edge_is_directed.to(device),
            )
            ...
            targets_node_predictions_signed = dataset.y_signed.to(device)
            targets_node_predictions_unsigned = dataset.y.to(device)
