#!/usr/bin/env python3
"""
Run finetuning sweeps for cities with completed pretraining checkpoints.

This script mirrors the orchestration logic from `run_and_finetune_all_cities.py`
but skips the initial training phase. It looks for `pretrain_{city}` checkpoints,
announces which ones exist, and launches two finetune runs for each:
    1. Continue from the pretraining checkpoint.
    2. Comparison run that starts from scratch.

Use the same CLI arguments as the combined script to reproduce data splits and
shared hyperparameters, while allowing different finetune-only overrides such as
`--finetune_limit_train_graphs` and `--finetune_limit_val_graphs`.
"""
...
DEFAULT_RESULTS_DIR = project_root / "inductive_gnn_data_results" / "transductive"
...
def create_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Finetune pre-trained models across all cities with updated settings."
    )
    ...
    # ------------------------------------------------------------------
    # Arguments specific to run_models.py (kept for shared defaults)
    # ------------------------------------------------------------------
    ...
    parser.add_argument("--finetune_limit_train_graphs", type=int, default=5)
    parser.add_argument("--finetune_limit_val_graphs", type=int, default=1)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override the base directory where checkpoints are stored.",
    )
    return parser
...
def determine_default_project_name(use_inductive_variant: bool) -> str:
    return "GNN_Inductive" if use_inductive_variant else "GNN_Transductive"


def pretrain_checkpoint_path(
    results_dir: Path,
    project_name: str,
    city: str,
) -> Path:
...
def main() -> None:
    ...
    default_project_name = determine_default_project_name(args.use_inductive_variant)
    ...
    if finetune_base_args.get("pretraining_inductive") is None:
        finetune_base_args["pretraining_inductive"] = args.use_inductive_variant

    finetune_base_args["project_name"] = project_name

    existing_pretrain_cities: List[str] = []
    missing_pretrain_cities: List[str] = []
    for city in city_sequence:
        checkpoint = pretrain_checkpoint_path(results_dir, project_name, city)
        if checkpoint.exists():
            existing_pretrain_cities.append(city)
        else:
            missing_pretrain_cities.append(city)

    print("=" * 80)
    if existing_pretrain_cities:
        print("Found completed pretraining runs for the following cities:")
        for city in existing_pretrain_cities:
            print(f"  - {city}")
    else:
        print("No completed pretraining runs found for the requested cities.")
    if missing_pretrain_cities:
        print("\nSkipping cities without checkpoints:")
        for city in missing_pretrain_cities:
            print(f"  - {city}")
    print("=" * 80)

    if not existing_pretrain_cities:
        print("Nothing to finetune. Exiting.")
        return

    print("Starting finetuning for completed cities listed above.\n")
    ...
    for idx, test_city in enumerate(city_sequence, start=1):
        ...
        if test_city not in existing_pretrain_cities:
            print("Pretraining checkpoint missing. Skipping finetune runs for this city.")
            continue

        shared_values = {name: getattr(args, name, None) for name in SHARED_ARG_NAMES}
