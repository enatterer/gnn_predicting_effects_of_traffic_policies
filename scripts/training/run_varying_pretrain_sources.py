...
For Landshut as the target city, this script systematically varies the number
...
Each pretraining run saves checkpoints, which are then used to finetune on Landshut
using the same train/val/test split from run_pretrain_finetune_comparison.py.

IMPORTANT: This script requires an existing train/val/test split for Landshut.
By default, it looks for: data/splits/landshut/rs_1/t40_v10/landshut_rs1_t40_v10_seed42_train40_val10_test100_distant_iou.json

To generate this split if it doesn't exist, run:
    python scripts/training/run_pretrain_finetune_comparison.py \\
      --testing_cities landshut --train_val_configs '40:10' --num_random_seeds 1 \\
      --skip_scratch True --skip_finetuning True

Example usage:
    python scripts/training/run_varying_pretrain_sources.py --gnn_arch trans_encoder

Example with custom split:
    python scripts/training/run_varying_pretrain_sources.py \\
      --train_val_size '80:20' --seed_idx 2

Example (nohup):
    nohup bash -lc 'source ~/.bashrc; PYTHONUNBUFFERED=1 stdbuf -oL -eL python -u scripts/training/run_varying_pretrain_sources.py --project_name VaryingPretrainSources' > nohup_varying_sources.log 2>&1 & echo $!
"""
...
TARGET_CITY = "landshut"

# Source cities available for pretraining (excluding Landshut)
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--split_file", type=str, default=None,
                        help="Path to existing train/val/test split file for Landshut. If not provided, auto-detects from splits_dir.")
...
def call_finetune_models(base_args: Dict[str, object]) -> None:
    """Call finetune_models.py to finetune on Landshut."""
...
def main() -> None:
    ...
    if args.split_file:
        ...
        if not split_file_path.exists():
            ...
            print(f"    --testing_cities landshut \\")
